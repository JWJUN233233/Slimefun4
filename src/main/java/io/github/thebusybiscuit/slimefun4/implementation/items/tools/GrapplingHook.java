    ...
    public @Nonnull ItemUseHandler getItemHandler() {
        return e -> {
            ...
            if (e.getClickedBlock().isEmpty()
                ...
                Arrow arrow = p.getWorld()
                        .spawn(
                                p.getEyeLocation().add(direction.getX(), direction.getY(), direction.getZ()),
                                Arrow.class);
                arrow.setShooter(p);
                ...
                Slimefun.getGrapplingHookListener()
                        .addGrapplingHook(p, arrow, bat, state, despawnTicks.getValue(), isConsumed);
            }
        };
    }
}
