...
class BackpackCommand extends SubCommand {
                ...
                if (args.length < 1) {
                    Slimefun.getLocalization()
                            .sendMessage(
                                    ...
                                    "messages.usage",
                                    true,
                                    msg -> msg.replace("%usage%", "/sf backpack <Player>"));
                    return;
                }
    ...
    private void showBackpackMenu(OfflinePlayer owner, Player p, Set<PlayerBackpack> result, int page) {
        var menu = new ChestMenu("Backpacks of " + owner.getName());
        ...
        for (int i = 0; i <= 36; i++) {
            ...
            lore.add("");
            lore.add(ChatColor.translateAlternateColorCodes('&', "&aClick to get the backpack"));
            im.setLore(lore);
